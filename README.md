...
The game ships with a **bundled GLB chess set** in `client/public/models/`
(glossy clearcoat materials, finer geometry) that loads through the standard
GLTF pipeline. It's CC0 and generated by [`scripts/export-models.mjs`](scripts/export-models.mjs) —
...
Every piece also has a **procedural fallback** (lathe-turned classic silhouettes
plus an extruded knight) baked into the code, so the board still renders
perfectly even if a model is missing or fails to load — zero broken-link risk.

**Want to use a different pack?** Drop any free `.glb`/`.gltf` set (e.g. from
Poly Pizza, Kenney, Quaternius or Sketchfab) into `client/public/models/` and
point `MODEL_SOURCES` at the top of
[`client/src/game/PieceFactory.ts`](client/src/game/PieceFactory.ts) at the new
files. The loader auto-scales and re-bases each model; no other changes needed.
